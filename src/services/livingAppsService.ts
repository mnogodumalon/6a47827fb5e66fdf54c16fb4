...
import type { Mitarbeiterverzeichnis, CreateMitarbeiterverzeichnis } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getMitarbeiterverzeichnis(): Promise<Mitarbeiterverzeichnis[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.MITARBEITERVERZEICHNIS}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Mitarbeiterverzeichnis[];
    return enrichLookupFields(records, 'mitarbeiterverzeichnis');
  }
  static async getMitarbeiterverzeichni(id: string): Promise<Mitarbeiterverzeichnis | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.MITARBEITERVERZEICHNIS}/records/${id}`);
    const record = { record_id: data.id, ...data } as Mitarbeiterverzeichnis;
    return enrichLookupFields([record], 'mitarbeiterverzeichnis')[0];
  }
  static async createMitarbeiterverzeichni(fields: CreateMitarbeiterverzeichnis) {
    return callApi('POST', `/apps/${APP_IDS.MITARBEITERVERZEICHNIS}/records`, { fields: cleanFieldsForApi(fields as any, 'mitarbeiterverzeichnis') });
  }
  static async updateMitarbeiterverzeichni(id: string, fields: Partial<CreateMitarbeiterverzeichnis>) {
    return callApi('PATCH', `/apps/${APP_IDS.MITARBEITERVERZEICHNIS}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'mitarbeiterverzeichnis') });
  }
  static async deleteMitarbeiterverzeichni(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.MITARBEITERVERZEICHNIS}/records/${id}`);
  }
