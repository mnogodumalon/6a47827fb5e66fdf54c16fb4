...
import type { Mitarbeiterverzeichnis, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Mitarbeiterverzeichnis';
...
interface MitarbeiterverzeichnisDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Mitarbeiterverzeichnis['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Omit<Mitarbeiterverzeichnis['fields'], 'abteilung'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  abteilung: LOOKUP_OPTIONS['mitarbeiterverzeichnis']?.['abteilung'] ?? [],
};
...
export function MitarbeiterverzeichnisDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: MitarbeiterverzeichnisDialogProps) {
  const [fields, setFields] = useState<Partial<Mitarbeiterverzeichnis['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['vorname', 'nachname', 'personalnummer', 'abteilung', 'email'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Mitarbeiterverzeichnis['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'mitarbeiterverzeichnis');
      await onSubmit(clean as Mitarbeiterverzeichnis['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "vorname": string | null, // Vorname\n  "nachname": string | null, // Nachname\n  "personalnummer": string | null, // Personalnummer\n  "abteilung": LookupValue | null, // Abteilung (select one key: "geschaeftsfuehrung" | "personal" | "finanzen" | "vertrieb" | "marketing" | "it" | "einkauf" | "logistik" | "recht" | "kundenservice" | "sonstige") mapping: geschaeftsfuehrung=Geschäftsführung, personal=Personal, finanzen=Finanzen, vertrieb=Vertrieb, marketing=Marketing, it=IT, einkauf=Einkauf, logistik=Logistik, recht=Recht, kundenservice=Kundenservice, sonstige=Sonstige\n  "telefon": string | null, // Telefon\n  "email": string | null, // E-Mail-Adresse\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Mitarbeiterverzeichnis['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Mitarbeiterverzeichnis bearbeiten' : 'Mitarbeiterverzeichnis hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'abteilung': (
      <div key="abteilung" className="space-y-1.5">
        <Label htmlFor="abteilung">Abteilung <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Select
    ...
    'email': (
      <div key="email" className="space-y-1.5">
        ...
        <Input
          id="email"
          type="email"
          placeholder="z. B. anna.schmidt@unternehmen.de"
          value={fields.email ?? ''}
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "personalnummer": "Personalnummer", "abteilung": "Abteilung", "telefon": "Telefon", "email": "E-Mail-Adresse"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.MITARBEITERVERZEICHNIS} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
