...
import { MitarbeiterverzeichnisViewDialog } from '@/components/dialogs/MitarbeiterverzeichnisViewDialog';
...
const MITARBEITERVERZEICHNIS_FIELDS = [
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('mitarbeiterverzeichnis');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'mitarbeiterverzeichnis': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'mitarbeiterverzeichnis': return (data as any).mitarbeiterverzeichnis as Mitarbeiterverzeichnis[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'mitarbeiterverzeichnis': return MITARBEITERVERZEICHNIS_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'mitarbeiterverzeichnis': return {
        create: (fields: any) => LivingAppsService.createMitarbeiterverzeichni(fields),
        update: (id: string, fields: any) => LivingAppsService.updateMitarbeiterverzeichni(id, fields),
        remove: (id: string) => LivingAppsService.deleteMitarbeiterverzeichni(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'mitarbeiterverzeichnis' || dialogState?.entity === 'mitarbeiterverzeichnis') && (
        <MitarbeiterverzeichnisDialog
          open={createEntity === 'mitarbeiterverzeichnis' || dialogState?.entity === 'mitarbeiterverzeichnis'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'mitarbeiterverzeichnis' ? handleUpdate : (fields: any) => handleCreate('mitarbeiterverzeichnis', fields)}
          defaultValues={dialogState?.entity === 'mitarbeiterverzeichnis' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Mitarbeiterverzeichnis']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Mitarbeiterverzeichnis']}
        />
      )}
      {viewState?.entity === 'mitarbeiterverzeichnis' && (
        <MitarbeiterverzeichnisViewDialog
          open={viewState?.entity === 'mitarbeiterverzeichnis'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'mitarbeiterverzeichnis', record: r }); }}
        />
      )}
